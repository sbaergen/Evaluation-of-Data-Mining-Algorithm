...
public class Main {
    ...
    public static void main(String args[]) {
        ...
        m.diffAttrProb = false; //Set to true to set different attribute bernoulli probability for patterns
        m.patAttrProb = 0; //Sets the pattern attribute bernoulli probability (only used if prev. line set to true
        if (args.length == 2)
    ...
    /**
     * Creates patterns by creating small EFGs and stores in file
     * @param filename
     * @return
     */
    public Vector<EFG> createPatterns(String filename){
        ...
        //Uncomment this code to use patterns from a file
        /*if (file.exists()){
        ...
        String tempAttrProb = patternValues.get(9);
        if (diffAttrProb) {
            patternValues.set(9, patAttrProb + "");
        }
        createGraph(patternValues, patterns, true);
        createGraphFile(filename, patterns);
        position = 12;
        if (diffAttrProb)
            patternValues.set(9, tempAttrProb);
        return patterns;
    }
    ...
    public void addEdges(EFG efg, double edgeProb){
        boolean limit = true; //Whether to limit edges or not
        int edgeLimit = 2; //Edge limit.
